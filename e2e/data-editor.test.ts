...
async function withDataEditorPage(run: (page: Page) => Promise<void>) {
	...
	const page = await dataEditorContext.newPage();
	try {
		await page.goto('/app/games');
		await run(page);
	} finally {
...
function dataEditorTest(name: string, run: (page: Page) => Promise<void>) {
	test(name, async () => {
		await withDataEditorPage(run);
	});
}
...
test.describe.serial('data editor', () => {
	test.beforeAll(async ({ browser }, testInfo) => {
		dataEditorContext = await browser.newContext({
			baseURL: testInfo.project.use.baseURL as string | undefined
		});
		const page = await dataEditorContext.newPage();
		try {
			await seedDataEditorProjects(page);
		} finally {
			await page.close();
		}
	});
	...
	test.afterAll(async () => {
		await dataEditorContext?.close();
		dataEditorContext = null;
	});

	dataEditorTest('insert rows in spreadsheet editor', async (page) => {
	...
	dataEditorTest('go to spreadsheet editor', async (page) => {
	...
	dataEditorTest('append buttons add one row and one column', async (page) => {
		await openWesternDataEditor(page);
	...
	dataEditorTest(
		'creating decks resets the form and accepts decimal comma dimensions',
		async (page) => {
			await page.getByRole('main').getByText('western-cards').click();
			await page.getByRole('button', { name: 'Decks' }).click();
	...
	dataEditorTest('creates a pre-made French playing card deck', async (page) => {
		await page.getByRole('main').getByText('western-cards').click();
		await page.getByRole('button', { name: 'Decks' }).click();
		await openNewDeckDialog(page);
	...
	dataEditorTest('creates a generated number and color deck', async (page) => {
		await page.getByRole('main').getByText('western-cards').click();
		await page.getByRole('button', { name: 'Decks' }).click();
		await openNewDeckDialog(page);
		...
		await expect
			.poll(() =>
				opfsEntryExists(page, '/western-cards/assets/premade-decks/number-color/red_1.svg')
			)
			.toBe(true);
	...
	dataEditorTest('creates a pre-made French Tarot deck', async (page) => {
		await page.getByRole('main').getByText('western-cards').click();
		await page.getByRole('button', { name: 'Decks' }).click();
		await openNewDeckDialog(page);
