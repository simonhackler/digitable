...
export async function initComponent(
    ...
    if (component.type == 'stack') {
        ...
        for (const id of stack.componentIds) {
            ...
            if (!boardGameItems.has(id)) {
                await initComponent(deps, hybridResults, state.components.get(id)!, state, s, room, selectionManager);
            }
    ...
    itemContainer.on('pointerdown', (e) => {
        if (e.ctrlKey) {
            if (selectionManager.has(boardGameItem) && e.button === 1) {
                selectionManager.deselect(boardGameItem);
            } else {
                selectionManager.select(boardGameItem);
            }
        } else {
            selectionManager.selectOnly(boardGameItem);
        }

        // Let's think about this if it makes sense.
        e.stopPropagation();
