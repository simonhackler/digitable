...
async function createImageSprites(svg: SVGSVGElement, texture: Texture, imageInfos: ImageInfo[]): Promise<Sprite[]> {
    ...
    for (const imageInfo of imageInfos) {
        try {
            ...
            if (imageInfo.transform) {
                ...
                if (transformMatch) {
                    const [rawTx, rawTy = 0] = transformMatch[1]
                        .split(',')
