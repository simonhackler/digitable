<script lang="ts">
	...
	let { view, state }: { view?: EditorView; state?: EditorState } = $props();
...
<div class="flex flex-wrap items-center gap-1" role="toolbar" aria-label="Markdown editor toolbar">
	...
	<button
		...
		aria-label="Quote"
		title="Quote"
		onclick={() => state && run(wrapIn(state.schema.nodes.blockquote))}><Quote size={16} /></button
	>
	<button
