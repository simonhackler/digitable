...
const layoutNodeKinds = new Set<LayoutNodeKind>([
	'table',
	'slot',
	'hand',
	'component',
...
function getOrCreateComponentPosition(
	...
	const position =
		state.positions.get(componentId) ??
		createPositionNode(componentId, kind, x, y, visible, parentId);
...
function createCardState(
	...
	const cardPosition = getOrCreateComponentPosition(
		state,
		cardId,
		'component',
		x,
...
function acceptedCardMatches(acceptedCardId: string, componentId: string, componentName: string) {
	return (
		...
		(componentName ? acceptedCardId === `${componentName}:${componentId}` : false)
	);
}
...
function layoutAcceptsComponent(target: LayoutNode, component: Component) {
	...
	if (component.componentName && layout.acceptedDeckNames.includes(component.componentName))
		return true;
...
function validateInitLayoutNodes(
	layoutNodes: unknown
): layoutNodes is InitLayoutNodePayload[] | undefined {
	...
	for (const node of layoutNodes) {
		...
		if (node.layout !== undefined) {
			...
			if (node.layout.maxChildren !== undefined && !isFiniteCoordinate(node.layout.maxChildren))
				return false;
			if (
				node.layout.acceptedDeckNames !== undefined &&
				!Array.isArray(node.layout.acceptedDeckNames)
			)
				return false;
			if (node.layout.acceptedCardIds !== undefined && !Array.isArray(node.layout.acceptedCardIds))
				return false;
		}
	...
	for (const node of layoutNodes) {
		...
		if (typeof parentId === 'string' && parentId !== '' && !layoutNodeIds.has(parentId))
			return false;
	}
...
function validateInitTableItems(
	...
	for (const item of tableItems) {
		...
		if (
			...
			(typeof item.parentId !== 'string' || !layoutNodeIds.has(item.parentId))
		) {
			return false;
		}
